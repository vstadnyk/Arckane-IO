...
class <? echo $helper->className() ?> extends <? echo $helper->parentClassName() ?> {
    function getGramInfoHeaderSize() {
        return <? echo $helper->getInfoHeaderSize() ?>;
    }
    ...
    function readGramInfoHeader($offset) {
        ...
        $result = unpack(
            ...
            <? echo $helper->storage->read('$offset', 2 * 10) ?> 
        );
    ...
    protected function readAncodesMap($info) {
        ...
        $offset = $info['offset'] + <? echo $helper->getInfoHeaderSize() ?> + $info['forms_count'] * 2;
                
        <? echo $helper->out($helper->storage->seek('$offset'), ';') ?> 
            
        $forms_count = $info['packed_forms_count'];
        return unpack("v$forms_count",  <? echo $helper->storage->read('$offset', '$forms_count * 2') ?>);
    }
    ...
    function readAncodes($info) {
        ...
        $offset = $info['offset'] + <? echo $helper->getInfoHeaderSize() ?>;

        <? echo $helper->out($helper->storage->seek('$offset'), ';') ?> 
            
        $forms_count = $info['forms_count'];
        $ancodes = unpack("v$forms_count", <? echo $helper->storage->read('$offset', '$forms_count * 2') ?>);
    ...
    function readFlexiaData($info) {
        ...
        <? echo $helper->out($helper->storage->seek('$offset'), ';') ?> 
        return explode($this->ends, <? echo $helper->storage->read('$offset', '$info[\'affixes_size\'] - $this->ends_size') ?>);
    }
    ...
    protected function readSectionIndex($offset, $count) {
        ...
        return array_values(unpack("V$count", <? echo $helper->storage->read('$offset', '$count * 4') ?>));
    }
    ...
    function readAllPartOfSpeech() {
        ...
        foreach($this->readSectionIndexAsSize($this->header['poses_index_offset'], $this->header['poses_count'], $this->header['poses_size']) as $size) {
            ...
            $res = unpack(
                'vid/Cis_predict',
                <? echo $helper->storage->read('$offset', 3) ?> 
            );
            
            $result[$res['id']] = array(
                'is_predict' => (bool)$res['is_predict'],
                'name' => $this->cleanupCString(<? echo $helper->storage->read('$offset + 3', '$size - 3') ?>)
            );
    ...
    function readAllAncodes() {
        ...
        for($i = 0; $i < $this->header['ancodes_count']; $i++) {
            $res = unpack('vid/vpos_id', <? echo $helper->storage->read('$offset', 4) ?>);
            $offset += 4;
            
            list(, $grammems_count) = unpack('v', <? echo $helper->storage->read('$offset', 2) ?>);
            $offset += 2;
            
            $result[$res['id']] = array(
                'pos_id' => $res['pos_id'],
                'grammem_ids' => $grammems_count ? 
                    array_values(unpack("v$grammems_count", <? echo $helper->storage->read('$offset', '$grammems_count * 2') ?>)) :
                    array(),
                'offset' => $offset,
            );
